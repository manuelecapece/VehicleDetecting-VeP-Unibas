...
class LineCounterAnnotatorBus:
    ...
    def annotateBus(self, frame: np.ndarray, line_counter_bus: LineCounterBus) -> np.ndarray:
        """
        ...
        cv2.line(
            frame,
            line_counter_bus.vector.start.as_xy_int_tuple(),
            line_counter_bus.vector.end.as_xy_int_tuple(),
            self.color.as_bgr(),
            self.thickness,
        ...
        in_text = f"Bus in: {line_counter_bus.in_count}"
        out_text = f"Bus out: {line_counter_bus.out_count}"
